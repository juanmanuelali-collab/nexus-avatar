...
function setDropzoneUploading(dzId, isUploading) {
  const dz = $(dzId);
  const emptyEl = dz.querySelector('.dz-empty');
  const previewEl = dz.querySelector('.dz-preview');
  if (isUploading) {
    dz.classList.add('is-uploading');
    dz.dataset.originalText = emptyEl.textContent;
    emptyEl.innerHTML = '<span class="dz-spinner"></span> Subiendo…';
    emptyEl.style.display = 'flex'; // por si ya habia una preview y el "vacio" estaba oculto
  } else {
    dz.classList.remove('is-uploading');
    if (dz.dataset.originalText) emptyEl.textContent = dz.dataset.originalText;
    // Si la subida fallo y ya habia una foto de antes, volvemos a ocultar el "vacio"
    // (si tuvo exito, setPreview/setDraftPreview ya se encargan de esto por su cuenta).
    emptyEl.style.display = previewEl && !previewEl.hidden ? 'none' : '';
  }
}

function wireDraftDropzone(kind, inputId, dzId) {
  $(dzId).addEventListener('click', () => {
    if ($(dzId).classList.contains('is-uploading')) return; // evita doble click mientras sube
    $(inputId).click();
  });
  $(inputId).addEventListener('change', async (e) => {
    const file = e.target.files[0];
    if (!file || !state.draftProject) return;
    setDropzoneUploading(dzId, true);
    try {
      if (kind === 'photo') {
      ...
      setDropzoneUploading(dzId, false);
    }
  });
}
