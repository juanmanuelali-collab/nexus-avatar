...
const VALID_CATEGORIES = ['residencial', 'comercial', 'rural', 'especifico'];
...
router.post('/', requireAuth, async (req, res) => {
  const {
    name, description, project_type, project_category, sector, style, budget_level, input_mode = 'photo',
  ...
  if (project_category && !VALID_CATEGORIES.includes(project_category)) {
    return res.status(400).json({ error: `project_category inválida: ${project_category}` });
  }
...
router.patch('/:id', requireAuth, async (req, res) => {
  const {
    purpose_tags, maintenance_level, keep_elements, name, style, budget_level,
    keep_elements_list, do_not_modify, location,
    project_category, project_type, sector,
  } = req.body;
  ...
  if (keep_elements_list !== undefined) updates.keep_elements_list = keep_elements_list;
  if (do_not_modify !== undefined) updates.do_not_modify = do_not_modify;
  if (location !== undefined) updates.location = location || null;
  if (name !== undefined) updates.name = name;
  if (style !== undefined) updates.style = style;
  if (budget_level !== undefined) updates.budget_level = budget_level;
  if (project_category !== undefined) updates.project_category = project_category || null;
