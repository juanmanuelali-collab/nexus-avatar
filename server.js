...
const GROK_MODEL       = 'xai/grok-imagine-video';
...
// ── GROK IMAGINE VIDEO ────────────────────────────────────────
app.post('/api/grok-generate', auth, async (req, res) => {
  try {
    const { prompt, imageBase64, imageMimeType, duration, resolution, aspectRatio, mode } = req.body;
    ...
    // Si hay imagen, agregarla como image-to-video
    if (imageBase64) {
      input.image = `data:${imageMimeType || 'image/jpeg'};base64,${imageBase64.replace(/^data:[^;]+;base64,/, '')}`;
    }

    console.log('[grok] Iniciando generación | duración:', input.duration, 's | resolución:', input.resolution);

    const createRes = await fetch('https://api.replicate.com/v1/models/' + GROK_MODEL + '/predictions', {
      method: 'POST',
      headers: {
        'Authorization': 'Bearer ' + REPLICATE_TOKEN,
        'Content-Type':  'application/json',
    ...
    console.log('[grok] Predicción creada:', prediction.id, '| status:', prediction.status);
    res.json({ ok: true, predictionId: prediction.id, status: prediction.status, output: prediction.output });
  } catch(e) {
