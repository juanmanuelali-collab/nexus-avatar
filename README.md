...
✅ Controles avanzados: descripción libre, "qué evitar" (positivizado vía Claude), paleta de color, encuadre, clima de luz, aspect ratio, profundidad de campo — prompt armado como JSON estructurado
✅ Landscape Brief inteligente (uso del espacio, mantenimiento, qué conservar) con autoguardado
✅ Site Analysis automático vía Claude vision al subir la foto real (siempre como estimación)
✅ 3 conceptos diferenciados por generación (Contemporánea/Natural/Premium), cada uno con su propio polling
✅ Auditorías 69 (quality real vía resolution/output_quality) y 70 (input_images confirmado) — cerradas
...
## Brief inteligente + Site Analysis + 3 conceptos (secciones 52/53/57)

- **Landscape Brief** (sección 53): a nivel *proyecto*, no por generación — para qué se usa el espacio, nivel de mantenimiento deseado, qué conservar. Se autoguarda al tocar cualquier campo (`PATCH /api/projects/:id`) y se incorpora a TODAS las generaciones de ese proyecto vía `promptEngine.js`.
- **Site Analysis** (sección 52): apenas se sube la foto real, corre en background una llamada a Claude vision (`src/lib/ai/siteAnalysis.js`) que detecta piscina/césped/vegetación/superficies existentes. Siempre se guarda y se muestra como **estimación**, nunca como medición certificada — el frontend hace polling liviano un par de veces después del upload, sin bloquear nada.
- **3 conceptos diferenciados** (sección 57): cada `POST /generate` ahora dispara 3 predictions en paralelo (Contemporánea / Natural / Premium — `modelConfig.concepts`), cada una con un `styleEmphasis` propio inyectado al prompt. Se muestran como 3 tarjetas independientes con su propio polling; el costo en créditos es 3x, chequeado antes de arrancar. Si un concepto falla, los otros dos siguen su curso — no es todo-o-nada.

## Auditorías 69/70 (correcciones críticas del pedido) — CERRADAS

- **69 (quality)**: el parámetro sí viajaba bien de punta a punta, pero `standard`/`highQuality` apuntaban al mismo modelo. Resuelto usando `resolution` + `output_quality` como el diferenciador real (más simple que sostener dos slugs de modelo — separar `flux-2-pro`/`flux-2-max` queda para un benchmark aparte, sección 67/68).
- **70 (campo de multi-imagen)**: confirmado con una prediction real — `input_images` era correcto. También se sumaron `resolution` y `output_quality`, confirmados en la misma llamada.

## Profundidad de campo

Checkbox "Estilo foto profesional" en el formulario — cuando está activo, el prompt pide `camera.depth_of_field` (fondo desenfocado, sujeto nítido), aprovechando que el modelo soporta ese control de cámara según su propia documentación.

## Primera tanda de la spec ampliada de inputs (Fase 1)

Organizado por dónde vive cada dato, priorizando UX (no todo amontonado en un formulario único):

- **En la creación del proyecto**: Categoría (Residencial/Comercial/Rural/Espacio específico) → Subtipo → Sector, con cascada condicional en JS (`CATEGORY_DATA` en `app.js`) — cambiar la categoría repuebla los otros dos selects.
- **En el Brief (autoguardado, nivel proyecto)**: Ubicación, "Elementos a conservar" y "Elementos que la IA NO debe modificar" como checkboxes reales (antes solo había un textarea libre) — se traducen en instrucciones explícitas y separadas del prompt (`elements_to_keep` vs `do_not_modify`, este último es una restricción dura). Función del espacio ampliada con más opciones.
- **En "Contanos qué buscás" (por generación)**: Nivel de intervención (slider 0-100, con hint descriptivo en vivo — nunca afloja la preservación arquitectónica, eso sigue siendo absoluto en el prompt independientemente del nivel) y criterios de vegetación (autóctona/bajo mantenimiento/bajo consumo de agua/alta biodiversidad).

Pendiente de la spec ampliada (Fase 2 y 3): clima, orientación solar, materiales, circulación, riego, estación del año, y el "Modo Proyecto" completo — quedan para más adelante.

## Fixes: borrado de proyectos, overflow mobile, refuerzo anti-invención

- **Borrado de proyectos**: nunca existía — se agregó `DELETE /api/projects/:id` (cascada automática sobre `generations`/`project_references` vía `ON DELETE CASCADE`, ya configurado en la DB) + botón "Eliminar proyecto" en el detalle, con confirmación. Los archivos ya subidos a Storage NO se borran automáticamente (limitación conocida, aceptable por ahora).
- **Overflow horizontal en mobile**: bug real de CSS — los elementos dentro de `.elements-grid` (grid de 2 columnas) tenían `min-width: auto` por default, así que una etiqueta larga ("Árboles y arbustos") empujaba la página entera a ser más ancha que la pantalla en vez de partirse en dos líneas. Se agregó `min-width: 0` + `overflow-wrap: break-word` en todos los contenedores flex/grid del formulario (`.elements-grid label`, `.chip`, `.field-row .field`, `.mode-option`, `.segmented-btn`), más una red de seguridad global (`overflow-x: hidden` en `html, body`) para que este tipo de bug nunca vuelva a romper toda la página aunque aparezca en un componente futuro.
- **Refuerzo anti-invención**: el prompt (`preserve`/`constraints`) ahora es explícito sobre conteo y dimensiones exactas de ventanas/puertas/elementos estructurales — no solo "preservá la arquitectura" en general, sino "la cantidad, tamaño y posición exactos deben coincidir, no agregues/quites/redimensiones ninguna abertura". **Sigue sin ser una garantía dura** (ver nota en la sección de flux-2-pro más arriba) — es reducción de riesgo vía mejor prompting, no un bloqueo técnico.

## Home comercial + Workspace — dos identidades separadas

- **`/`** — home comercial pública (`public/index.html`, `css/home.css`, `js/home.js`). Paleta clara/natural (ivory, verde salvia, gris piedra), tipografía editorial — pensada para convertir visitantes (Arquitectos/Paisajistas) en usuarios. Sin lógica de negocio, solo presentación + slider Antes/Después + scroll reveal.
- **`/app`** — el workspace real (login, proyectos, generación) — se movió de la raíz a `public/app/`. Mantiene la identidad oscura/técnica original. `express.static` resuelve la redirección `/app` → `/app/` sola, sin rutas extra en `server.js`.
- Ambas comparten `GET /config.js` (inyecta `SUPABASE_URL`/`SUPABASE_ANON_KEY` al navegador).

**Confirmado**: las 3 fotos "después" (Patio residencial, Terraza urbana, Piscina) son renders reales generados con AI Landscape Designer — copy y badges ("Después · IA") en `index.html` reflejan esto con tranquilidad, no es una afirmación sin respaldo. El flujo del Hero (chips "Original / Concepto / AI Render") sigue mostrando solo etiquetas de texto sobre una foto — pendiente sumar imágenes reales para esa secuencia cuando estén disponibles.
